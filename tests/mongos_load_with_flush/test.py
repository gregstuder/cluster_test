...
provisioner = provisioning.AWS('mongos_load_with_flush', credentials_dir='./aws_test')
...
chaos_pm = pms[4]
chaos_shell = MongoShell(chaos_pm, 'shell_start_chaos', cluster.mongoses[0], \
                         './chaos.js', options={ 'waitFor' : 5 * 60 * 1000 + 10000, 'sleepTime' : 1 * 60 * 1000 }, \
                         isFile=True, version='2.0.7')
    
chaos_shell.gen_command(init_shell.last_phase + 1)
