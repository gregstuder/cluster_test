...
def GenIthInstance(i):
    base   = [ "template<typename Target, typename Res%s>", #1 ,typename types
               ...
               "",
               "private:",
               "  TargetFunc _target_func; // not owned here",
               "  Target* _obj;            // not owned here",
               "%s",                                        #12 bound defn
               "};",
               "" ]
    ...
    one    = [     "Res ret = ((*_obj).*_target_func)(%s);", # bound + unbound
               "    delete this;",
               "    return ret;" ]

    void   = [     "((*_obj).*_target_func)(%s);",           # bound + unbound
               "    delete this;" ]

    many   =       "return ((*_obj).*_target_func)(%s);"     # ditto
    ...
    while True:
        for once in [True, False]:
            ...
            bound_name_u = ["_bind%d" % x for x in range(1, bound+1)]
            ...
            init = [skip6+"_%s(%s)" % (bound_name[x], bound_name[x]) for x in rgn]
            decl = ["  %s _%s;" % (bound_tp[x], bound_name[x]) for x in rgn]
